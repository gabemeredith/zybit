...
| **Shipped (PR #66)** | Sprint 3 follow-up: AI Variant Advisor surface | **Zybit-143** — `extractDesignTokens` (pure) derives a compact token set co-written atomically with the design-snapshot row in `buildFullDesignSnapshot`. **Zybit-144** — `POST /api/dashboard/experiments/ai-suggest` calls Gemini 2.0 Flash via REST, returns up to 3 schema-valid `VariantModification[]` proposals; strict validation (CTA+forms selector allowlist — headings out of scope; `attribute-set` attribute allowlist; `css-inject` content checks; `text-replace` sanitisation; prompt-injection-resistant prescription delimiters); `element-reorder` deliberately excluded; returns 503 without `GEMINI_API_KEY`. **Zybit-148** — per-org daily rate limit (10 calls/org/UTC day, atomic upsert on `phase2_ai_advisor_usage` per migration `0018`; denied calls don't bump counter) + structured cost logging under `service: 'ai-advisor'`. **Proposals only — nothing applies them to a live DOM yet (Zybit-149 client-side runtime not built).** Migration `0018` must be applied to Neon before the route goes live. |
