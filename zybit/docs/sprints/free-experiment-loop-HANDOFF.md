...
- **Phase 4 (next, first GUI-visible piece):** create a `previewOnly` experiment from `proposeFreeExperiment` + `projectImpact`, and render it on `/app/loop` (`src/app/app/loop/page.tsx`) as a **"Projected"** timeline entry (distinct from a measured outcome). The `preview_only` proxy exclusion already guarantees it never serves real traffic. *Clear of PR 107.*
...
- **Verification style:** the founder (Gabe) verifies by clicking a GUI and seeing output, not by reading test logs. Phase 4 / 7 are the first things he can actually click — drive him to a runnable surface to confirm, don't just show green tests.
- Everything built so far is backend modules + the gate; the proposal/projection/`preview_only` are **not yet wired into a user-facing flow** — Phase 4 is where it becomes visible.
