...
**Context A/B eval** (`lighthouse/lib/eval/`, `web` Context A/B panel) — the "is SiteContext noticeably better?" surface. Enter a URL → it runs the SAME audit twice, baseline (`siteContext:false`) vs enriched (`siteContext:true`), Layer B + deriveFacts forced on so every finding gets comparable prose, pairs findings by `ruleId|pathRef`, and renders the prose side by side. Per-finding **valid/invalid** buttons persist to a file-backed verdict store (`POST /lighthouse/api/eval/verdict`); the **scoreboard** shows baseline vs enriched valid-rate and the delta. This is the human gate for flipping `LLM_SITE_CONTEXT_ENABLED` in prod — the existing `layerB/eval` pairwise LLM-judge stays as a secondary signal. The `siteContext?: boolean` override (beats the env flag, like `layerB`) is what makes the A/B race-free in one process.
...
1. **AI Variant Advisor is effectively dead on real-site audits.** Every CTA/form/heading gets `cssSelector: null`, so the advisor's allowlist is empty → 0 options. **Root cause (confirmed):** `computeCssSelector` (`src/lib/phase2/snapshots/cssSelector.ts`) only emits a selector for a stable anchor — `data-testid`/`data-test`/`data-qa`/`data-cy`, a human-authored `#id`, `[name]`, or `[role][aria-label]` — and **bails to `null` with no positional fallback** (deliberate, to avoid fragile class-chain selectors). cohor7/commitmint elements are bare `<a>`/`<h1>` with none of those, so `null`. This is **not** a URL-audit-vs-in-app difference — both paths run the same parser; the advisor is equally blocked in-app on sites without test anchors. A `ref`-based fallback does **not** work: `data-zybit-ref` isn't on the live DOM, so the proxy couldn't apply the mod. **FIXED (commit f1bfc30):** added a positional `nth-of-type` parent-path rung to `computeCssSelector` (anchored on the nearest stable ancestor or `body`), tiered `fragile`. Live-verified: cohor7's advisor now returns 3 options/finding (`#root > … > h1:nth-of-type(1)`). Caveat: positional selectors don't round-trip through the flat `buildMinimalHtml`, so the experiment-builder validation badge + selector-staleness cron can't verify them (they work on the live DOM, where the proxy applies them) — minor follow-up.
2. **fix-preview render came back blank on cohor7** (commitmint rendered fine). The new screenshot quality gate (`gpt-5.4-mini`) correctly caught it (`issue: "blank"`) and suppressed before spending budget — the gate works. **Root cause (confirmed):** `renderOne` (`src/lib/audit/fixPreview/renderBeforeAfter.ts:333`) builds `originUrl` from the **apex** `phase1Sites.domain` (`https://cohor7.com/`), `page.route()`-intercepts that exact host+path and fulfills the **static snapshot HTML shell**, then `goto`s. For a JS-rendered/redirecting SPA the served shell never hydrates/paints → blank. The vision pass succeeds because `captureAboveFoldBuffer` navigates the **resolved final URL** with no interception, so the real server + JS paint normally. commitmint's above-fold paints from the shell; cohor7's needs JS. **PARTIAL FIX (commit 5d35267):** the render now targets the snapshot's resolved `finalUrl` (post-redirect) instead of the apex domain — fixes blanks caused by apex→host redirects loading assets from the wrong origin. **Residual (root cause now confirmed):** cohor7 is still blank, but **not** from hydration (a JS-disabled render was tried and reverted — it made no difference). The real cause: `renderBeforeOnly` (`renderBeforeAfter.ts:419`) does a **plain HTTP fetch** of the URL and then **`stripScripts`**. For a client-rendered React SPA, the raw HTML is an empty `#root` shell (content only appears after JS), so the rendered "before" is blank. The *audit* found content because `runSnapshot` renders SPAs via **Browserless**; fix-preview's plain fetch doesn't. **Proper fix (larger follow-up):** render the "before" through Browserless (or reuse the Browserless-rendered snapshot HTML) for SPA shells, then `stripScripts` the rendered DOM. Touches the shared public-`/audit` render path. Meanwhile the quality gate fail-safes (no blank preview ships).
