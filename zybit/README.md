...
| `AUTH_FROM_EMAIL` | Auth emails | `from` address for welcome / set-password emails (must be a verified Resend sender). Defaults to `Zybit <noreply@mail.getzybit.com>` — a dedicated sending subdomain keeps transactional reputation off the root domain. |
| `INTAKE_NOTIFY_EMAIL` | Request-access form | Where the "Request access" notification email lands. Defaults to `asad@getzybit.com`. |
...
| `OPENAI_API_KEY` | All AI surfaces (non-essential — each degrades gracefully) | Powers every AI call, routed through `src/lib/ai/openai.ts`: AI Variant Advisor (`POST /api/dashboard/experiments/ai-suggest` — 503 when unset, PMs fall back to manual entry; per-org daily rate limit 10 calls/UTC day via `phase2_ai_advisor_usage`, migration `0018`), capture-time copy critique + visual signals, audit vision pass (`visionPass.ts`), and the fix-preview advisor / image inpaint / screenshot quality gate. Replaced the Gemini integration 2026-05. |
| `OPENAI_REASONING_MODEL` / `OPENAI_FAST_MODEL` / `OPENAI_IMAGE_MODEL` | Optional model overrides | Default `gpt-5.4` (reasoning: variant + audit fix advisor) / `gpt-5.4-mini` (fast: capture-time extraction + screenshot gate) / `gpt-image-1` (Tier-2 inpaint). |
...
| `NEXT_PUBLIC_BOOK_CALL_URL` | Public URL-audit client pages | Calendly link used as the book-call CTA on `/audit/[id]` and `/app`. Falls back to a hardcoded default when unset. Set alongside `ZYBIT_BOOK_CALL_URL` so email and web CTAs stay in sync. |
...
| `PUBLIC_AUDIT_TEST_EMAILS` / `NEXT_PUBLIC_AUDIT_TEST_EMAILS` | Public URL-audit (dev/staging/prod debug) | Comma-separated personal-email allowlist that bypasses the work-email gate at `/audit`. All other gates (SSRF, rate limits, $25/day budget, double opt-in) still apply, so abuse is bounded by inboxes you actually own. The two vars are **unioned** on the server (not precedence-ordered) so the GUI form and the API agree on which addresses pass; empty values are treated the same as unset. `PUBLIC_AUDIT_TEST_EMAILS` is server-only (sufficient for curl-driven testing); `NEXT_PUBLIC_AUDIT_TEST_EMAILS` is also inlined into the client bundle so the GUI form's pre-submit check accepts the address (only put addresses here whose ownership is already public). Production-safe — leaving these set in prod just allows debug submissions from those addresses without changing any other behavior. |
