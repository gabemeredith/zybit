...
| **Observability** | ✅ Built | Cronitor + error budget + structured logger wired into crons; Axiom drain connected — best-effort fire-and-forget ingest in `logger.ts`, active when `AXIOM_TOKEN`+`AXIOM_DATASET` set; console JSON output preserved for platform drains. **Axiom verified (2026-05-21):** token `xaat-f3c5f5b7...` confirmed working; dataset `axiom-audit` confirmed writeable. Set `AXIOM_DATASET=axiom-audit` in Vercel env vars to activate (Zybit-153 gate criteria met). |
...
> **Status (2026-05-21):** Sprint 0 (PR #49) + Sprint 1 (PR #51) + Sprint 2 (PR #53) merged to main. Sprint 3 (Layer 2 calibration) on `claude/learn-layer-2-calibration-3A7e9` (PR #54): Layer 2 shipped + PM-visible surface (Zybit-164 equivalent) + Lighthouse Layer 2 exercise. Sprint 0/1/2 remediation also landed on this branch: full-loop E2E regression net (Zybit-120), PostHog bridge health probe (Zybit-126), Edge Config kill-switch (Zybit-116), selector-staleness cron (Zybit-133), copy-quality hints (Zybit-125), per-path snapshot drift (Zybit-135), Segment schema guard (Zybit-137), selector stability tiers (Zybit-134). CI: 43 test files, 489 tests passing.
...
| GA4 measurement limitation warning (Zybit-157) | ⬛ Not built | GA4-only customers will silently get no outcomes. Must show amber banner. |
...
3. **GA4 limitation warning** (Zybit-157, ~0.5d): If only GA4 connected → amber banner "Measurement loop requires PostHog or Segment." Prevents silent 0-outcome runs.
4. **Connector circuit breaker** (Zybit-154, ~2d): After 10 consecutive sync failures → pause + ops email. Prevents silent retry loops that consume quota.
