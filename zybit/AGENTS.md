...
| **Learn** (outcome feedback loop) | ✅ Built (L1+L2) | Layer 1 shipped — per-site re-ranking via `applyLearnRerank` (`src/lib/phase2/rules/learnReranker.ts`): cascade match (Tier 1–4) + D-with-guardrails formula. Wired into `runInsightsPipeline`, persisted as `learn_adjustment` jsonb on `forge_findings` (drizzle/0013), surfaced as backlog pill, finding-detail "Past tests" panel, and LEARNED timeline entry. **Layer 2 shipped** — per-site rule-threshold calibration (`ruleCalibration.ts`): `computeRuleCalibrations` aggregates outcomes per `ruleId` into a detection-floor multiplier (`clamp(1 − netSignal, 0.7, 1.3)`, gated at 3+ conclusive outcomes); 11 of 12 rules (set in `CALIBRATED_RULE_IDS`; `hero-hierarchy-inversion` exempt — sample-size-only gate) route their detection floor through `calibratedFloor`/`calibratedCap`; computed in `runInsightsPipeline` and surfaced in `AuditRuleDiagnostic.calibration`. **Unit-verified, not Lighthouse-verified** (the runner resets outcomes before its single insights pass, so it can't exercise calibration). No PM-facing surface yet — findings appear/disappear silently. Layer 3 (cross-site priors) deferred until 50+ customers. |
...
> **Status (2026-05-21):** Sprint 0 (PR #49) + Sprint 1 (PR #51, Zybit-121/122/125) merged to main; Sprint 2 (snapshot refresh cron, Lighthouse Phase 2, Axiom drain) merged via PR #53. Sprint 3 in progress on `claude/learn-layer-2-calibration-3A7e9`: **Learn — Layer 2 (per-site rule-threshold calibration)** shipped. CI: 37 test files, 446 tests passing.
...
2. ~~**Learn — Layer 2 (per-site rule-threshold calibration)**~~ **Shipped** — `ruleCalibration.ts` mutates per-site detection floors from outcome history; wired into `runInsightsPipeline`, all 12 rules calibrated. Layer 3 (cross-site priors) deferred until 50+ customers.
